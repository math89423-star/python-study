...
```Cookie```：“身份证明”。
2. 响应（Response） -- 服务器返回的包裹
服务器收到单子后，会回给一个包裹
**Status Code（状态码）**：此时最应该关注的数字
```python
200 OK : 成功！已经拿到货了
403 Forbidden : 禁止访问（通常是由于反爬，或者IP被封了）
404 Not Found : 没找到（资源删除了，ID出错了）
500/502/503 : 仓库自己炸了（服务器崩溃）
```

# 链表（Linked List）
## 形象理解
### 数组与链表s
**数组（Array/List）:**电影院的座位
-   特点：所有人（数据）都是紧挨着的，位置是固定的连续区域
-   查找快：想找第3个人，直接去第3个座位即可（时间复杂度O(1)）
-   插入慢：如果要在第2个位置上塞一个人进去，所有人都需要起身向后挪动（时间复杂度O(n)）
**链表（Linked List）:**寻宝游戏
-   特点：数据散落在内存的各个角落
-   结构：每个人（节点Node）手里只拿着两样东西
    1.自己的情报（数值val）
    2.下一级特务的地址（指针Next）
-   查找慢：你想找第三个人，必须先问第 1 个人“下线是谁？”，找到第 2 个，再问第 2 个“下线是谁？”，最后才能找到第 3 个 (O(n))。
-   插入快： 你想在 A 和 B 之间插一个人 C，只需要让 A 修改手里的纸条指向 C，C 手里的纸条指向 B 即可。不需要任何人移动位置 (O(1))。
## 代码定义
Python中没有专门的数据类型来定义链表，需要自己定义一个类来实现
```python
class ListNode:
    def __init__(self, val=0, next=None):
        self.val = val  # 数据域：存值
        self.next = next # 指针域：存下一个节点的引用

# 操作1：链表的遍历
def print_list(head):
    current = head # 创建一个指针，指向头节点
    while current is not None: # 只要指针指向不为空
        print(current.val) # 打印出当前节点的值
        current = current.next # 指针向后移动，跳到下一个节点

# 操作2：插入节点,假设在prev_node后插入new_node
new_node.next = prev_node.next # 新节点先牵住后面节点的手
prev_node.next = new_node   # 前面的节点再牵住新节点的手

# 操作3：删除节点，假设是1->2->3，变成1->3
...
dummy.next = head
"""
目的：为了解决头结点也是变动的问题，我们人为地在链表最前面造一个“假头”。
真实链表：1 -> 2 -> 3
Dummy(0) -> 1 -> 2 -> 3
好处：无论怎么删改真实的数据，Dummy 永远在最前面。最后返回 dummy.next 就是新的头节点。
"""
```
